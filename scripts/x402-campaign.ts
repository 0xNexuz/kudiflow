...
import { buySignal, getBalances, runProcurementAgent, type SignalServiceName } from "../src/agent.js";

interface Args {
  runs?: number;
  currentCount?: number;
  targetCount?: number;
  signal: SignalServiceName | "all";
  maxUsdc: number;
...
function parseArgs(argv: string[]): Args {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (flag === "--signal") args.signal = signalName(value);
...
function signalName(value: string): SignalServiceName | "all" {
  if (value === "stock" || value === "delivery" || value === "risk" || value === "all") return value;
  throw new Error("--signal must be one of: stock, delivery, risk, all.");
}
...
async function preflightSignalServices(config: ReturnType<typeof loadConfig>, signal: SignalServiceName | "all"): Promise<void> {
  const entries = signal === "all"
    ...
    : [[signal, config.signalServices[signal]]] as const;
...
async function main(): Promise<void> {
  ...
  const signalsPerRun = args.signal === "all" ? Object.keys(config.signalServices).length : 1;
  const costPerRun = args.signal === "all"
    ? Object.values(config.signalServices).reduce((total, service) => total + signalPriceUsdc(service.price), 0)
  ...
  for (let runIndex = 1; runIndex <= runs; runIndex += 1) {
    ...
    try {
      ...
      console.log(`run ${runIndex}/${runs}: ${receipts.length} x402 receipts, total ${successfulPayments}`);
    } catch (error) {
